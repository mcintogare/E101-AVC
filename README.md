...
--- | --- | ---
 ...
 ## Installation guide
These are instructions to install and run the robot program. These instructions are for Windows machines.

#### Installing Geany
1) Download the Geany application [here](https://download.geany.org/geany-1.36_setup.exe)

2) Once the download is complete, click on it, and follow the instructions that are given.

#### Installing MinGW C++ compiler
1) Download the set up .exe file for MinGW C++ compiler [here](http://www.rose-hulman.edu/class/csse/binaries/MinGW/mingw-get-inst-20110530.exe). The download should start automatically.

2) When the download is finished, at the bottom left you will see **mingw-get-setup.exe**. Open it.

3) A window will pop up asking if you want to install, click **Install**.

4) Next, it will ask if you want to install MinGW in `C:\\MinGW`, this is fine so click **Continue**.

5) After the installation has finished (it will say 100%), click **Continue** to be prompted to the next window.
