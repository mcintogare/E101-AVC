class RobotCompletion {

public:
	double vLeft, vRight, v_go;
	double kp = 0.1;
	
	RobotCompletion(double velocityLeft, double velocityRight, double goSpeed){
		vLeft = velocityLeft;
		vRight = velocityRight;
		v_go = goSpeed;
	}
	
	double getvLeft(){ return RobotCompletion::vLeft; }
	double getvRight(){ return RobotCompletion::vRight; }
	double getvGo(){ return RobotCompletion::v_go; }
	
	void changevLeft(double dV){
		double v_go = getvGo();
		RobotCompletion::vLeft = v_go + dV;
		std::cout<<"left "<<RobotCompletion::vLeft<<std::endl;
	}
	
	void changevRight(double dV){
		double v_go = getvGo();
		RobotCompletion::vRight = v_go - dV;
		std::cout<<"right "<<RobotCompletion::vRight<<std::endl;
	}
	
	int brightness(ImagePPM cameraView, int pixIndex, int depth);
	double offset(ImagePPM cameraView, int depth);
	
	void runRobotCompletion();
};
 ...
 */
int RobotCompletion::brightness(ImagePPM cameraView, int pixIndex, int depth) {
 ...
 */
double RobotCompletion::offset(ImagePPM cameraView, int depth){
...
void RobotCompletion::runRobotCompletion() {
